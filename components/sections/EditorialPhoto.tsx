...
export default function EditorialPhoto({ image, className, sizes, preload = false }: {
  ...
  return (
    <div className={cn("relative overflow-hidden bg-wash", className)}>
      {image.src ? (
        <Image src={image.src} alt={image.alt} fill sizes={sizes} preload={preload} className="object-cover" style={{ objectPosition: image.position }} />
      ) : (
